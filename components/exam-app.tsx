...
function PickView({ onStart, records }: { onStart: (id: string) => void; records: Record<string, { bestScore: number; attempts: number }> }) {
  ...
  return <><div className="mb-8 flex flex-wrap items-end justify-between gap-4"><div><p className="mb-2 font-mono text-xs uppercase tracking-[.18em] text-muted-foreground">01 / Choose your exam</p><h2 className="text-2xl font-semibold tracking-tight">Four exams. One calm scoreboard.</h2></div><span className="rounded-full border border-border px-4 py-2 text-xs text-muted-foreground">{done}/{exams.length} exams taken · {total} questions</span></div><div className="grid gap-4 md:grid-cols-2">{exams.map((exam, index) => { const record = records[exam.id]; return <motion.button key={exam.id} onClick={() => onStart(exam.id)} whileHover={{ y: -3 }} className="rounded-3xl border border-border bg-card p-7 text-left transition-colors hover:border-primary"><div className="mb-10 flex items-center justify-between"><span className="font-mono text-xs text-primary">0{index + 1} / exam</span><span className="grid size-8 place-items-center rounded-full bg-primary/10 text-primary"><CircleHelp className="size-4" /></span></div><div className="flex items-end justify-between gap-4"><div><h3 className="text-xl font-medium tracking-tight">{exam.title}</h3><p className="mt-2 max-w-sm text-sm leading-6 text-muted-foreground">{exam.description}</p></div></div><div className="mt-8 flex flex-wrap items-center gap-4 border-t border-border pt-5 text-xs text-muted-foreground"><span className="inline-flex items-center gap-1.5"><Timer className="size-3.5 text-primary" />{exam.duration}</span><span>{exam.questions.length} questions</span>{record && <span className="ml-auto font-mono text-primary">{record.bestScore}/{exam.questions.length} best</span>}</div></motion.button> }) }</div></>
}
...
function ResultView({ exam, answers, records, onRetry, onPick }: { exam: Exam; answers: Record<string, number>; records?: { bestScore: number; attempts: number }; onRetry: () => void; onPick: () => void }) {
  ...
  return <div className="mx-auto max-w-3xl"><div className="rounded-3xl border border-border bg-card p-7 text-center md:p-12"><div className="mx-auto mb-6 grid size-16 place-items-center rounded-full bg-primary/10 text-primary"><Award className="size-8" /></div><div className="font-mono text-xs uppercase tracking-[.2em] text-primary">{exam.title}</div><h2 className="mt-3 text-3xl font-semibold tracking-tight md:text-5xl">{summary.score}<span className="text-xl text-muted-foreground md:text-3xl">/{exam.questions.length}</span> correct</h2><p className="mt-3 text-muted-foreground">{summary.pct}% · {summary.verdict}</p><div className="mt-6 flex flex-wrap items-center justify-center gap-3 text-xs"><span className="inline-flex items-center gap-1.5 rounded-full border border-border px-4 py-2 text-muted-foreground"><Trophy className="size-4 text-primary" />Best {records?.bestScore ?? summary.score}/{exam.questions.length}</span><span className="rounded-full border border-border px-4 py-2 text-muted-foreground">{records?.attempts ?? 1} attempt{(records?.attempts ?? 1) > 1 ? 's' : ''}</span></div></div><div className="mt-6 grid gap-3 sm:grid-cols-2">{summary.correct.map((item) => <div key={item.question.question} className={`rounded-2xl border p-5 ${item.correct ? 'border-border bg-card' : 'border-destructive/40 bg-destructive/5'}`}><div className="mb-3 flex items-center gap-2 text-xs font-medium">{item.correct ? <Check className="size-4 text-primary" /> : <X className="size-4 text-destructive" />}<span className="font-mono uppercase tracking-wider text-muted-foreground">Q{exam.questions.indexOf(item.question) + 1}</span></div><p className="text-sm leading-6">{item.question.question}</p>{!item.correct && <p className="mt-2 text-xs text-muted-foreground">Answer: {LETTERS[item.question.answer]}. {item.question.options[item.question.answer]}</p>}<p className="mt-3 border-t border-border pt-3 text-xs leading-5 text-muted-foreground">{item.question.explanation}</p></div>)}</div><div className="mt-8 flex flex-wrap justify-center gap-3"><button onClick={onRetry} className="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground"><RotateCcw className="size-4" />Try again</button><button onClick={onPick} className="inline-flex items-center gap-2 rounded-full border border-border px-5 py-3 text-sm font-medium text-muted-foreground transition-colors hover:text-foreground"><ArrowLeft className="size-4" />All exams</button></div></div>
}
